...
function Diff (Model, initialSettings) {
  ...
  const WILDCARD = '*';
  //                              Hook phase   Any hook phase           Operation              Any op.         Prop RegExp       Prop & wildcards       /mods
  //                              ----------   --------------           ---------              -------         -----------       ----------------       -----
  //                                  ⬇             ⬇                      ⬇                   ⬇                 ⬇                  ⬇                ⬇
  const HOOK_REGEX_PATTERN = `^((before|after|\\${WILDCARD}):)?((added|updated|set|deleted|\\${WILDCARD}):)?(\\/[^\\/]*\\/\\w*|[\\w\\.\\${WILDCARD}]+)(\\/\\w+)?$`;
  const HOOK_REGEX = new RegExp(HOOK_REGEX_PATTERN);

  const REGEX_KEY_REGEX_PATTERN = '^\\/([^\/]*)\\/(\\w*)$';
  const REGEX_KEY_REGEX = new RegExp(REGEX_KEY_REGEX_PATTERN);

  const DEFAULT_PHASE = 'before';
  const OPTS_SKIP = 'skipDiff';
  const HOOK_STATE_DIFF = 'diff';
  const HOOK_STATE_PREV_DATA = 'prevData';
  ...
  Model.once('attached', () => {
    ...
    Model.observe('before save', async (ctx) => {
      ...
      let inst = ctx.instance || ctx.currentInstance;
      let prevInst = inst;
      /* istanbul ignore else */
      if (inst) {
        _.set(inst, INST_EXEC_HOOKS, []); // Reset list of executed hooks
        ...
        if (ctx.data) {
          ...
          if (ctx.isNewInstance) {
            ...
            // Instance is the one being updated and didn't got patch data
            // Instance is actually the mutated instance ready to save
            // Need to fetch the previous data from database to diff
            prevInst = await Model.findById(inst.getId());
            prevData = prevInst.toJSON();
          }
        }
        ...
        _.set(ctx, `hookState.${HOOK_STATE_PREV_DATA}`, prevData);
        // Remember instance before applying changes
        _.set(ctx, `hookState.${HOOK_STATE_PREV_INST}`, prevInst);
        // Remember diff
    ...
    Model.observe('after save', async (ctx) => {
      ...
      let inst = ctx.instance || ctx.currentInstance;
      if (inst) {
        // Recalculate diff because it could change during the previous phase execution
        const prevData = _.get(ctx, `hookState.${HOOK_STATE_PREV_DATA}`);
        const newData = cleanDataObject(inst.toJSON());
        const diff = calculateDiff(prevData, newData);
        _.set(ctx, `hookState.${HOOK_STATE_DIFF}`, diff);
        // Execute hooks
    ...
    Model.prototype.executeDiffHooks = async function (phase, diff, ctx) {
      ...
      if (skipOpts !== true) {
        ...
        let data, finalData;
        do {
          data = cleanDataObject(this.toJSON()); // Keep to compare at the end of the iteration
          ...
          for (let [changeSet, props] of Object.entries(diff)) {
            ...
            for (let [prop, value] of Object.entries(props)) {
              ...
              for (let hook of hooks) {
                ...
                /* istanbul ignore else */
                if (_.isFunction(f)) {
                  const helpers = buildHelpers(this, ctx);
                  const hookResult = await f.apply(this, [value, prop, helpers, prevInst, ctx]);
          ...
          // Calculate iteration diff to detect changes produced within the triggered hooks
          finalData = cleanDataObject(this.toJSON());
          diff = calculateDiff(data, finalData);
        } while (!diffIsEmpty(diff));
      }
...
//============================================================================
  ...
  /**
   * Whether a diff result is empty
   * @param {object} diff
   * @return {boolean}
   */
  function diffIsEmpty (diff) {
    return _.every(diff, _.isEmpty);
  }
  ...
  /**
   * Build helper functions which can be useful inside hooks
   * @param {Model} inst Subject instance
   * @param {object} ctx Hook context
   */
  function buildHelpers (inst, ctx) {
    return {
      extendAttributes: (attributes) => {
        // If the invoked method was updateAttributes,
        // the passed instance proceeds from "currentInstance", which is read-only.
        // In that case, ctx.data should be used to mutate the instance to be saved.
        // This helper method is intended to handle the different cases
        _.extend(inst, attributes);
        if (ctx.data) {
          _.extend(ctx.data, attributes);
        }
      },
    };
  }
  ...
  /**
   * Detect if a str is a RegExp pattern and return it
   * @param {string} str
   * @return {RegExp|null}
   */
  function detectRegExp (str) {
    const [, pattern, flags] = REGEX_KEY_REGEX.exec(str) || [];
